...
def fq2bam(argList):
    bsmb_parser = argparse.ArgumentParser(
        description="Run GPU-bwa mem, co-ordinate sorting, marking duplicates, and Base Quality Score Recalibration to convert FASTQ to BAM/CRAM.",
        formatter_class=PbHelpFormatter.PbHelpFormatter, #argparse.ArgumentDefaultsHelpFormatter,
        usage='''pbrun fq2bam [<args>]\nHelp: pbrun fq2bam -h''')
    IOOptions = []
    IOOptions.append(PBOption(category="IOOption", name="--ref", helpStr="Path to the reference file.", required=True,
    ...
    IOOptions.append(PBOption(category="IOOption", name="--in-se-fq-list",
                              ...
                              typeName=IsFileReadable))
    IOOptions.append(PBOption(category="IOOption", name="--in-se-bam", helpStr="Path to a single-ended BAM/CRAM file that will be used to convert back to FASTQ as the input of fq2bam.",
