...
def run_scheduler_tmp(ref, bam, outfile_name, partition, gpus_per_partition, streams_per_gpu, run_partition, alt_contigs, binary, imported_vcf, gvcf_output, htvc_para):
	...
	for i in range(0, int(partition)):
		...
		htvc_cmd = "CUDA_VISIBLE_DEVICES=" + devices + " " + binary + " " + ref + " " + bam + " " + str(gpus_per_partition) + " " + str(streams_per_gpu) + " -o " + tmp_file + " " +  all_interval_arg + " " + htvc_para
		print(htvc_cmd)
...
def run_scheduler(ref, bam, outfile_name, partition, gpus_per_partition, streams_per_gpu, run_partition, alt_contigs, binary, imported_vcf, gvcf_output, htvc_para):
	...
	for i in range(0, len(all_interval_arg)):
		if len(all_interval_arg[i]) > 0:
			...
			if gvcf_output == True:
				...
				tmp_file = str(i) + ".vcf"
			htvc_cmd = "CUDA_VISIBLE_DEVICES=" + devices + " " + binary + " " + ref + " " + bam + " " + str(gpus_per_partition) + " " + str(streams_per_gpu) + " -o " + tmp_file + " " +  all_interval_arg[i] + " " + htvc_para
			print(htvc_cmd)
			process = subprocess.Popen(htvc_cmd, shell=True)
			all_processes.append(process)
			all_tmp_files.append(tmp_file)
...
if __name__ == "__main__":
	...
	if (len(sys.argv) < 9):
		print("run ./scheduler deepvariant_binary ref bam numberOfGPUS streamsPerGPU -o outfile --model model_file  other_htvc_parameters")
		exit(1)
	...
	streams_per_gpu = int(sys.argv[5])
	run_partition = False
	gvcf_output = False
	imported_vcf = ""
	read_tmp = False

	for i in range(8, 8 + len(sys.argv[8:])):
		if (sys.argv[i] == "-L"):
			user_interval = True
			i = i + 1
	...
	if (run_partition == True and len(imported_vcf) > 0 and gvcf_output == True):
		print("--run-partition cannot work with --proposed_variants in gvcf mode, turn of partition")
		run_partition = False
	...
	if run_partition:
		...
		if user_interval:
			...
			exit(1)
		if (sys.argv[7].endswith('.gz')):
			print("--run-partition cannot work with .gz output, exit...")
			exit(1)
		if gvcf_output == True:
			if (sys.argv[7].endswith('.g.vcf') != True):
	...
	if read_tmp == True:
		run_scheduler_tmp(sys.argv[2], sys.argv[3], sys.argv[7], partition_num, gpus_per_partition, streams_per_gpu, run_partition, alt_contigs, sys.argv[1], imported_vcf, gvcf_output, ' '.join(sys.argv[8:]))
	else:
		run_scheduler(sys.argv[2], sys.argv[3], sys.argv[7], partition_num, gpus_per_partition, streams_per_gpu, run_partition, alt_contigs, sys.argv[1], imported_vcf, gvcf_output, ' '.join(sys.argv[8:]))
